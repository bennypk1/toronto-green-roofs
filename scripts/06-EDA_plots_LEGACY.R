...
# Setup plotting data
# plotting_subset <- analysis_data %>% filter(ECO_ROOF == "TRUE")
plotting_subset <- analysis_data
green_roof_distribution <- plotting_subset %>%
...
# Plot
ggplot() +
  geom_fsa(data = MFSA_data_augmented, fill = "TOTAL_AREA") +
  theme_classic() +
  labs(
